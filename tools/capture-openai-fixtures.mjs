...
const REQUIRED_CASES = [
  "models_json", "chat_headerless_json", "chat_stream_sse", "chat_stateful_first",
  "chat_stateful_newest", "chat_idempotent_retry", "responses_nonstored_json",
  "responses_stream_sse", "responses_stored_json", "responses_previous_json",
  "responses_no_idempotency",
];
const PINNED = {
  handler: ["src/websocket/app-server-openai.ts", "22058763c116e87ea0d8decaa675c8c4df711e51f4895936832455a0376c2849"],
  turn_bridge: ["src/websocket/app-server-openai-turn.ts", "08f445e1ed19304dff8227ae058e71f555ec69142b11e7c0073154e916eec531"],
  common: ["src/websocket/app-server-openai-common.ts", "3053fcf764a9048ffafefba74d780c40a35ca1ed88df0131d6bc4f40d8482ccd"],
  lockfile: ["bun.lock", "0a8cad33168b97cfd08958d29b853f683eff9a36f42d1709e761990a74adc26e"],
};
const repository = process.env.LETTA_CODE_REPOSITORY ?? "/Volumes/Delorean/code/five-letters/letta-code";
...
function run(command, args, options = {}) {
  return new Promise((resolvePromise, reject) => {
    ...
    const timer = setTimeout(() => { child.kill("SIGKILL"); reject(new Error(`command timed out: ${command}`)); }, COMMAND_TIMEOUT_MS);
    const stdout = [];
    const stderr = [];
...
async function verifyPinnedTree(tree) {
  ...
  if (!turn.includes("runTurnViaListenerRuntime") || !turn.includes("dispatchInboundMessageWhenReady")) {
...
function validateCases(cases) {
  ...
  if (names.length !== REQUIRED_CASES.length || REQUIRED_CASES.some((name) => !names.includes(name))) throw new Error("required baseline capture case missing");
  if (new Set(names).size !== names.length) throw new Error("duplicate capture case");
}
async function writeCorpus(cases, metadata) {
  ...
  for (const fixture of cases) {
    if (!fixture.name || !fixture.route || !fixture.mode || !fixture.execution) throw new Error("captured case incomplete");
    ...
    entries.push({ name: fixture.name, route: fixture.route, mode: fixture.mode, dependencies: fixture.dependencies, path, sha256: sha256(text) });
  }
  const index = {
    schema_version: 2, baseline_commit: BASELINE_COMMIT, baseline_tree: BASELINE_TREE,
    capture_command: "LOTTA_BUN=bun node tools/capture-openai-fixtures.mjs",
    runtime: metadata.runtime, sources: metadata.sources,
...
async function archiveBaseline(temp) {
  ...
  const treeHash = (await run("git", ["-C", repository, "show", "-s", "--format=%T", BASELINE_COMMIT])).trim();
  if (treeHash !== BASELINE_TREE) throw new Error(`baseline tree mismatch: ${treeHash}`);
...
async function main() {
  ...
  try {
    ...
    if (runtime !== "1.3.14") throw new Error(`Bun runtime mismatch: ${runtime}`);
    await run(bun, ["install", "--frozen-lockfile", "--ignore-scripts"], {
      cwd: tree, env: { ...process.env, HOME: join(temp, "home") },
    });
    ...
    validateCases(cases);
    const sources = Object.fromEntries(Object.entries(PINNED).map(([name, [path, hash]]) => [name, { path, sha256: hash }]));
    sources.capture_runner = { path: "tools/openai-capture-runner.ts", sha256: runnerHash };
    await writeCorpus(cases, { runtime: { name: "bun", version: runtime }, sources });
