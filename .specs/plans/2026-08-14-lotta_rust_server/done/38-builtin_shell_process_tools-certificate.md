...
- **O1 — The seven shell/process behaviors of §Rust built-ins execute: one-shot shell/exec, PTY session, background output, stdin, monitor, stop, and timeout**
  ...
  - *Status:* ☒ SATISFIED — `builtin::shell::execution` passes 9 production-wired cases; the combined shell selector passes 30/30.
...
- **O2 — Timeout is distinct from cancellation and both are bounded: `LOCAL_TOOL_EXECUTION_TIMEOUT_MS_DEFAULT` interrupts, and a per-tool override stays bounded**
  ...
  - *Status:* ☒ SATISFIED — `timeout_outcome`, `cancellation_outcome_differs`, and `override_bounded` pass 3/3 with `ToolOutcome::Timeout` distinct from `Interrupted`.
...
- **O3 — Child termination is two-stage: SIGTERM then SIGKILL after a 2,000 ms grace, and no orphan survives**
  ...
  - *Status:* ☒ SATISFIED — 2/2 two-stage-kill cases pass, including a responsive parent with a TERM-ignoring descendant and exact 1,999 ms plus 1 ms escalation.
...
- **O4 — Captured output is bounded at `CHILD_PROCESS_OUTPUT_BYTES_MAX` and the model-facing result clamps at the shell-family 30,000 characters with overflow written to a file**
  ...
  - *Status:* ☒ SATISFIED — 9/9 output-bound cases pass, covering 16 MiB aggregate/peak retention, raw admission, one-time 30,000-character clamp, durable overflow, sessions, stdin, and channels.

- **O5 — Meets the repo definition of done (tests, lint/format, named-constant limits — see plan.md baseline)**
  - *Claim:* The repo-wide gates named in the plan's definition-of-done baseline pass for this change.
  - *Evidence to collect:* Run `cargo fmt --all --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo nextest run --workspace --all-features`, and `cargo deny check` — expect exit code 0 from each. Read every constant this task introduces and confirm it is a `const` whose name puts units last (`development-guidelines.md` §Naming), and that each function the task added stays within 70 lines and 100 columns.
  - *Status:* ☒ SATISFIED — fmt, strict workspace Clippy, workspace nextest 1,231/1,231, private Rustdoc, and `cargo deny check` pass; all 19 touched Rust files meet file/function/column limits.
...
- **O6 — Reviewable: a reviewer runs `cargo nextest run -p lotta-tools -E 'test(builtin::shell::)'` and sees the seven behaviors, distinct timeout and cancellation outcomes, two-stage kill, and both output bounds pass**
  ...
  - *Status:* ☒ SATISFIED — clean reviewer ran the combined selector 30/30 and found no residual process or fixture leak.

## Regression check

For each unit this task changes, the validator traces one downstream caller:

- `crates/lotta-tools/src/sandbox/` (Task 35) launches these children; confirm `sandbox::workspace` still passes with the shell launcher attached : ☒ PRESERVED — 7/7.
...
VERDICT: ☒ DONE
CONFIDENCE: ☒ high
SUMMARY: Seven exact shell registrations execute through the Task 33 pipeline and Task 35 sandbox with real PTY sessions, bounded lifecycle and I/O, distinct timeout/cancellation, descendant-safe two-stage group termination, exact names/assets, and all repository gates passing.
