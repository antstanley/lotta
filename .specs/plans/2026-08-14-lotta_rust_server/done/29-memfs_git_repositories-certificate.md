...
- `cargo nextest run -p lotta-memfs -E 'test(labels::)'`: 6/6.
- `cargo nextest run -p lotta-memfs -E 'test(ops::)'`: 20/20, including all thirteen operations, validation matrix, push, worktree, symlink, and capability-race cases.
- `cargo nextest run -p lotta-memfs -E 'test(repo::frontmatter_fallback) + test(repo::no_implicit_remote) + test(repo::bounds)'`: 3/3.
- Aggregate certificate selector: 35/35; full `lotta-memfs`: 37/37 twice.
- Git-backed and fake MemFS contracts: 4/4.
- `cargo nextest run --workspace --all-features`: 863/863 twice.
- `cargo fmt --all --check`, strict workspace Clippy, strict Rustdoc, and `cargo deny check`: pass.

## Regression check

For each unit this task changes, the validator traces one downstream caller:

- `crates/lotta-testkit/src/contract/` (Task 09) `MemFsPort` suite now runs against this adapter; confirm it passes for both the fake and the Git-backed implementation : ☒ PRESERVED
...
VERDICT: ☒ DONE
CONFIDENCE: ☒ high
SUMMARY: All six obligations and the Git-backed/fake contract regression are satisfied. Exact evidence: labels 6/6; frontmatter/remote/bounds 3/3; operation and review cases 20/20; aggregate certificate 35/35; full lotta-memfs 37/37 twice; Git plus fake contracts 4/4; workspace nextest 863/863 twice; fmt, strict workspace Clippy, strict Rustdoc, and cargo deny pass.
