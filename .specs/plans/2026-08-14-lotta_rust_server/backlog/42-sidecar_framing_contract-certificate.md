...
- Task 09 fake clock/process harness and Task 05 extension bounds drive framing tests; run `cargo nextest run -p lotta-extensions -E 'test(sidecar::framing)'` plus `cargo nextest run -p lotta-testkit -E 'test(contract::)'` and confirm both pass : ☐ (PRESERVED / REGRESSION)
