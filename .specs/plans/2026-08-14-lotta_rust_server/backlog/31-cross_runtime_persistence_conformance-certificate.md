...
- `crates/lotta-store/src/` writers (Tasks 22–27) are exercised here; confirm `agent::corpus_round_trip`, `transcript::append`, and `side::paths` still pass : ☐ (PRESERVED / REGRESSION)
- `crates/lotta-memfs/src/prompt/` (Task 30) records are round-tripped; confirm `prompt::record_shape` still passes : ☐ (PRESERVED / REGRESSION)
