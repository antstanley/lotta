...
- `crates/lotta-app-server/src/ws/router.rs` (Task 20) is exercised end to end here; confirm `ws::ordering_invariants` still passes under the live harness : ☐ (PRESERVED / REGRESSION)
- `crates/lotta-runtime/src/turn/loop.rs` (Task 19) now runs behind a socket; confirm `turn::terminal_once` still passes : ☐ (PRESERVED / REGRESSION)
