...
fn scope() -> RuntimeScope {
    RuntimeScope::new(
        lotta_domain::AgentId::accept("task-agent").expect("agent id"),
        lotta_domain::ConversationId::accept("task-conversation").expect("conversation id"),
        None,
    )
}
...
async fn lifecycle_create_get_list_update_dependency() {
    ...
    let first = port.create(&scope(), create("first")).await.unwrap();
    let second = port.create(&scope(), create("second")).await.unwrap();
    assert_eq!(port.get(&scope(), &first.task_id).await.unwrap(), first);
    assert_eq!(port.snapshot(&scope()).await.unwrap().len(), 2);
    let updated = port
        .update(
            &scope(),
            UpdateInput {
...
async fn unknown_terminal_and_race_are_fixed() {
    ...
    assert_eq!(
        port.get(&scope(), "task_999").await,
        Err(TaskError::Unknown)
    );
    let record = port.create(&scope(), create("race")).await.unwrap();
    let left = Arc::clone(&port);
    let right = Arc::clone(&port);
    let id_left = record.task_id.clone();
    let id_right = record.task_id.clone();
    let first = tokio::spawn(async move {
        left.update(&scope(), status(id_left, TaskStatus::InProgress))
            .await
    });
    let second = tokio::spawn(async move {
        right
            .update(&scope(), status(id_right, TaskStatus::InProgress))
            .await
    });
    ...
    let completed = port
        .update(
            &scope(),
            status(record.task_id.clone(), TaskStatus::Completed),
        )
        .await
        .unwrap();
    assert_eq!(completed.status, TaskStatus::Completed);
    assert_eq!(
        port.update(&scope(), status(record.task_id, TaskStatus::Pending))
            .await,
...
async fn invalid_dependency_and_cycle_roll_back_every_state_counter() {
    ...
    let first = port.create(&scope(), create("first")).await.unwrap();
    let second = port.create(&scope(), create("second")).await.unwrap();
    let before = port.states.lock().await.get(&scope()).cloned().unwrap();
    let mut invalid = status(first.task_id.clone(), TaskStatus::InProgress);
    invalid.add_blocks = vec!["task_999".into()];
    assert_eq!(
        port.update(&scope(), invalid).await,
        Err(TaskError::Invalid)
    );
    assert_state_eq(port.states.lock().await.get(&scope()).unwrap(), &before);

    let mut edge = status(first.task_id.clone(), TaskStatus::InProgress);
    edge.add_blocks = vec![second.task_id.clone()];
    port.update(&scope(), edge).await.unwrap();
    let before_cycle = port.states.lock().await.get(&scope()).cloned().unwrap();
    let mut cycle = status(second.task_id, TaskStatus::InProgress);
    cycle.add_blocks = vec![first.task_id];
    assert_eq!(port.update(&scope(), cycle).await, Err(TaskError::Invalid));
    assert_state_eq(
        port.states.lock().await.get(&scope()).unwrap(),
        &before_cycle,
    );
}
...
async fn dependency_bound_rollback_and_delete_clean_reciprocals() {
    ...
    assert_eq!(port.update(&scope(), over).await, Err(TaskError::Limit));
    assert_state_eq(port.states.lock().await.get(&scope()).unwrap(), &before);

    port.update(&scope(), status(first.task_id.clone(), TaskStatus::Deleted))
        .await
        .unwrap();
    assert_eq!(
        port.get(&scope(), &first.task_id).await,
        Err(TaskError::Unknown)
    );
    let remaining = port.get(&scope(), &second.task_id).await.unwrap();
