...
pub(super) struct PatchError;

type FileError = PatchError;
...
fn parse_operations<'a>(lines: &[&'a str], control: Option<&()>) -> Result<Patch<'a>, FileError> {
    let mut operations = Vec::new();
    operations.try_reserve(8).map_err(|_| PatchError)?;
    let mut index = 1;
    let end = lines.len().checked_sub(1).ok_or(PatchError)?;
    let mut hunk_count = 0usize;
    while index < end {
        check(control);
        validate_operations(operations.len().checked_add(1).ok_or(PatchError)?)?;
        let (operation, next, added_hunks) = parse_operation(lines, index, end, control)?;
        hunk_count = hunk_count.checked_add(added_hunks).ok_or(PatchError)?;
...
fn parse_operation<'a>(
    ...
    let source = line.strip_prefix("*** Update File: ").ok_or(PatchError)?;
...
fn parse_add<'a>(
    ...
    while index < end && lines[index].starts_with('+') {
        ...
        content.try_reserve(1).map_err(|_| PatchError)?;
...
fn parse_update<'a>(
    ...
    while index < end && !lines[index].starts_with("*** ") {
        ...
        hunks.try_reserve(1).map_err(|_| PatchError)?;
        hunks.push(hunk);
        index = next;
    }
    if hunks.is_empty() {
...
fn parse_hunk<'a>(
    ...
    while index < end {
        ...
        old.try_reserve(1).map_err(|_| PatchError)?;
        new.try_reserve(1).map_err(|_| PatchError)?;
        ...
        count = count.checked_add(1).ok_or(PatchError)?;
        index += 1;
    }
    if count == 0 {
        return Err(PatchError);
...
fn split_lines(input: &str) -> Result<Vec<&str>, FileError> {
    ...
    lines.try_reserve_exact(count).map_err(|_| PatchError)?;
    lines.extend(
        input
            .split('\n')
...
mod tests {
    ...
    fn patch_envelope_and_count_boundaries() {
        for (validator, maximum) in [
            ...
            assert_eq!(validator(maximum + 1), Err(PatchError));
        }
    }
