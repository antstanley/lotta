//! llama.cpp local endpoint adapter.
...
/// Explicit llama.cpp chat-completions adapter.
#[derive(Clone, Debug)]
pub struct LlamaCpp {
...
impl LlamaCpp {
    /// Constructs a llama.cpp adapter for its pinned `OpenAI` chat endpoint.
    ///
    /// # Errors
...
impl ProviderPort for LlamaCpp {
    fn stream(
        &self,
